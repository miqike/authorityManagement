...
var curStatus = "init", curAsyncCount = 0, asyncForAll = false, goAsync = false, _expandeLevel = 1;
...
function onAsyncSuccess(event, treeId, treeNode, msg) {
    ...
    if(window._refreshNode != null && window._refreshNode != undefined) {
        var zTree = $.fn.zTree.getZTreeObj(treeId);
        var refreshNode = zTree.getNodeByParam(window._refreshNode.filterName, window._refreshNode.nodeKey, treeNode);
        if(refreshNode != null) {
            zTree.selectNode(refreshNode, false);
            window._refreshNode = null;
            window.__refreshNode = refreshNode;
        } else {
            zTree.expandNode(treeNode, true, false, false, true);
            var descendantKey = getDescendantKey(treeNode[window._refreshNode.filterName], window._refreshNode.nodeKey);
            var descendantNode = zTree.getNodeByParam(window._refreshNode.filterName, descendantKey, treeNode);
            zTree.expandNode(descendantNode, true, false, false, true);
        }
    }

    try {
        _onAsyncSuccess(event, treeId, treeNode, msg);
    } catch (e) {}
}
...
function levelExpand(zTree, level) {
    window._expandeLevel = level;
    expandNodes(zTree, zTree.getNodes());
    if (!goAsync) {
        curStatus = "";
    }
...
function expandNodes(zTree, nodes) {
    ...
    curStatus = "expand";
    if (nodes[0].level < _expandeLevel - 1) {
...
function refreshParentNode(treeName, filterName, nodeKey) {
    var treeObj = $.fn.zTree.getZTreeObj(treeName);
    var parentNode = treeObj.getNodeByParam(filterName, nodeKey, null).getParentNode();
    var parentKey = parentNode[filterName];
    refreshNode(treeName, filterName, parentKey);
}

function refreshNode(treeName, filterName, nodeKey) {
    window._refreshNode = {
        treeName:treeName, filterName:filterName, nodeKey:nodeKey
    }
    var ancestorNode = getAncestorNode(treeName, filterName, nodeKey);
    var treeObj = $.fn.zTree.getZTreeObj(treeName);
    treeObj.reAsyncChildNodes(ancestorNode, "refresh", false);
}

function getAncestorNode(treeName, filterName, nodeKey) {

    var ancestorNode = null;
    var treeObj = $.fn.zTree.getZTreeObj(treeName);
    var parentKey = getParentKey(nodeKey);
    var parentNode = null;
    while(true) {
        var parentNode = treeObj.getNodeByParam(filterName, parentKey, null);
        if(null != parentNode && parentNode.isParent) {
...
function getParentKey(nodeKey) {
    var parentKey;
    var nodeLength = nodeKey.length;
    var length = 0;
    for(var i=0; i<codeRule.length; i++) {
        var t = codeRule[i];
        if((length + t) == nodeLength) {
            parentKey = nodeKey.substr(0, length);
            return parentKey;
        } else {
            length += t;
        }
...
function getDescendantKey(ancestorKey, targetKey) {
    var offset = 0;

    var ancestorKeyLength = ancestorKey.length;
    for (var i = 0; i < codeRule.length; i++) {
        offset += codeRule[i];
        if (offset == ancestorKeyLength) {
            var descendantKey = targetKey.substr(0, offset + codeRule[i+1]);
