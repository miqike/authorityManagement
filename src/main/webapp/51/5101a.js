...
function openEtlTool() {
	$.getJSON("../user/" + userInfo.userId + "/all", null, function (response) {
		...
		var param = "lieFinancialValidate://1:" + response.userId + "&" + response.salt + "&" + response.password+"&"+qy.jhbh + "&" + qy.hcdwXydm + "&" + qy.hcdwName+"&"+qy.jhmc+"&"+qy.jhnd+"&"+qy.hcfl==1?"定向":"不定向"+"&"+qy.hgjgmc+"&"+qy.zfryName1+"&"+qy.fr+"&"+qy.fr;
		console.log(param);
